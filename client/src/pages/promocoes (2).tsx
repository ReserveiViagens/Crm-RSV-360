...
export default function PromocoesPage() {
  const { data: cmsContent } = useCMSContent("promocoes")
  const { data: galleryData } = usePageGallery("promocoes")
    ...
    <div data-cms-section="hero" style={{
      ...
      color: "#fff", padding: "104px 20px 0",
        ...
        <div style={{ maxWidth: 780, margin: "0 auto", textAlign: "center" }}>
          ...
          <h1 data-testid="text-page-title" style={{ fontSize: "clamp(26px, 5vw, 38px)", fontWeight: 900, margin: "0 0 10px", lineHeight: 1.2, color: cmsHero?.textColor ?? undefined }}>
            {cmsHero?.headline ?? "Promoções Especiais"}
          </h1>
          <p style={{ fontSize: "clamp(14px, 2.5vw, 16px)", color: "rgba(255,255,255,0.75)", margin: "0 0 24px", lineHeight: 1.6 }}>
            {cmsHero?.subheadline ?? "Caldas Novas e Rio Quente com cupom exclusivo CALDAS15 — pacotes, hotéis e ingressos"}
          </p>
  ...
  return (
    <>
      ...
      <PageGallery images={galleryImages} video={galleryVideo} title="Galeria de promoções e ofertas" />
      </PublicPageShell>
    </>
  )
}
