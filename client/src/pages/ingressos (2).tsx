...
export default function IngressosPage() {
  const { data: cmsContent } = useCMSContent("ingressos")
  const { data: galleryData } = usePageGallery("ingressos")
    ...
    >
      ...
      <HotelCategoryNav
        categories={[
          ...
          { label: "Parques",    value: "Parques",    icon: Waves,      filterUpdate: {}, testId: "button-filter-parques" },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h1 style={{ fontSize: "clamp(26px, 5vw, 38px)", fontWeight: 800, margin: "0 0 8px", color: cmsHero?.textColor ?? undefined }} data-testid="text-page-title">
          {cmsHero?.headline ?? "Ingressos para Parques"}
        </h1>
        <p style={{ fontSize: "clamp(14px, 2.5vw, 16px)", opacity: 0.9, margin: "0 0 8px" }}>
          {cmsHero?.subheadline ?? "Até 29% OFF + Entrada prioritária"}
        </p>
      ...
      <PageGallery images={galleryImages} video={galleryVideo} title="Galeria de parques e ingressos" />
    </CatalogPageShell>
  )
}
