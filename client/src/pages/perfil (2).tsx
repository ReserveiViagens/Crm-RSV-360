...
import { ArrowLeft, User, Star, MapPin, Settings, ChevronRight, Shield, Award, Bell, LogOut, Home, Search, CalendarDays, Mail, Phone, Loader2, Camera, CheckCircle2, Trophy, Plus, BarChart3, Users, FileText, Send, type LucideIcon } from "lucide-react";
...
export default function PerfilPage() {
  const { data: cmsContent } = useCMSContent("perfil")
  ...
  const { data: myRequestData } = useQuery<{ id: string; status: string; message: string } | null>({
    queryKey: ["/api/access-requests/me"],
    enabled: !!user,
    retry: false,
  });

  const requestEditorMutation = useMutation({
    mutationFn: async (message: string) => {
      const res = await apiRequest("POST", "/api/access-requests", { message });
      return res.json();
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/access-requests/me"] });
    },
  });
  ...
  const isOrganizador = user?.role === "LIDER" || user?.role === "admin" || user?.role === "superadmin";
  const isAdmin = user?.role === "admin" || user?.role === "superadmin";
  const canRequestEditor = user && user.role !== "admin" && user.role !== "superadmin" && user.role !== "editor";
  ...
  return (
    <div className="rsv-subpage rsv-app-shell" style={{ background: theme?.backgroundColor ?? "#F9FAFB", minHeight: "100vh" }}>
      <div data-cms-section="hero" style={{
        background: getCMSHeroBg(cmsHero, "linear-gradient(135deg, #1e3a5f 0%, #2563EB 100%)"),
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          ...
          <span style={{ fontSize: 16, fontWeight: 900 }}>{cmsHero?.headline ?? <>RSV<span style={{ color: "#F57C00" }}>360</span></>}</span>
          {user ? (
      ...
      <div style={{ padding: "0 16px", marginTop: -20 }}>
        ...
        {user && (
          <>
            ...
            {canRequestEditor && (
              <div style={{ background: "#fff", borderRadius: 16, overflow: "hidden", marginBottom: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.08)", border: "1px solid #EDE9FE" }}>
                <div style={{ padding: "16px 20px 8px", display: "flex", alignItems: "center", gap: 8 }}>
                  <Shield style={{ width: 18, height: 18, color: "#7C3AED" }} />
                  <h2 style={{ fontSize: 16, fontWeight: 700, color: "#1F2937", margin: 0 }}>Acesso de Editor</h2>
                </div>
                <div style={{ padding: "8px 20px 20px" }}>
                  {myRequestData ? (
                    <div style={{ padding: 14, borderRadius: 10, background: myRequestData.status === "pending" ? "#FEF3C7" : myRequestData.status === "approved" ? "#DCFCE7" : "#FEE2E2" }}>
                      <p style={{ margin: "0 0 4px", fontSize: 13, fontWeight: 700, color: myRequestData.status === "pending" ? "#92400E" : myRequestData.status === "approved" ? "#166534" : "#dc2626" }}>
                        {myRequestData.status === "pending" ? "Solicitação enviada — aguardando aprovação" : myRequestData.status === "approved" ? "Acesso aprovado!" : "Solicitação rejeitada"}
                      </p>
                      <p style={{ margin: 0, fontSize: 12, color: "#6b7280" }}>
                        {myRequestData.status === "pending" ? "Um administrador irá revisar em breve." : myRequestData.status === "approved" ? "Você agora tem acesso de editor ao conteúdo do site." : "Entre em contato com um administrador."}
                      </p>
                    </div>
                  ) : (
                    <>
                      <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 12, lineHeight: 1.5 }}>
                        Solicite acesso de editor para contribuir com o conteúdo do site.
                      </p>
                      <textarea
                        data-testid="input-request-message"
                        value={requestMessage}
                        onChange={(e) => setRequestMessage(e.target.value)}
                        placeholder="Por que você quer ser editor? (opcional)"
                        rows={3}
                        style={{ width: "100%", padding: "10px 12px", border: "1px solid #d1d5db", borderRadius: 8, fontSize: 13, outline: "none", resize: "none", boxSizing: "border-box", marginBottom: 10 }}
                      />
                      <button
                        data-testid="button-solicitar-editor"
                        onClick={() => requestEditorMutation.mutate(requestMessage)}
                        disabled={requestEditorMutation.isPending}
                        style={{ width: "100%", padding: "11px 0", borderRadius: 10, border: "none", background: requestEditorMutation.isPending ? "#c4b5fd" : "linear-gradient(135deg, #4C1D95, #7C3AED)", color: "#fff", fontSize: 14, fontWeight: 700, cursor: requestEditorMutation.isPending ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
                      >
                        {requestEditorMutation.isPending ? <Loader2 style={{ width: 16, height: 16, animation: "spin 1s linear infinite" }} /> : <Send style={{ width: 16, height: 16 }} />}
                        {requestEditorMutation.isPending ? "Enviando..." : "Solicitar Acesso de Editor"}
                      </button>
                    </>
                  )}
                </div>
