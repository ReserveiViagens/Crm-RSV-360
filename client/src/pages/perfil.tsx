...
import { ArrowLeft, User, Star, MapPin, Settings, ChevronRight, Shield, Award, Bell, LogOut, Home, Search, CalendarDays, Mail, Phone, Loader2, Camera, CheckCircle2, Trophy, Plus, BarChart3, Users, FileText } from "lucide-react";
...
export default function PerfilPage() {
  ...
  const menuPassageiro = MENU_PASSAGEIRO.map((item) => {
              ...
              <Link key={i} href={item.href} style={{ textDecoration: "none" }}>
                <div data-testid={`menu-item-${item.label.toLowerCase().replace(/\s+/g, "-")}`} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: i < menuPassageiro.length - 1 ? "1px solid #F3F4F6" : "none" }}>
                  ...
                  {"badge" in item && (item as any).badge && (
                    <span style={{ background: "#EF4444", color: "#fff", padding: "1px 8px", borderRadius: 10, fontSize: 11, fontWeight: 700 }}>{(item as any).badge}</span>
                  )}
