...
export default function LeiloesPage() {
  const { data: cmsContent } = useCMSContent("leiloes")
  const { data: galleryData } = usePageGallery("leiloes")
  ...
  return (
    <div style={{ background: "#0F172A", minHeight: "100vh", position: "relative" }}>
      ...
      <div data-cms-section="hero" style={{
        background: getCMSHeroBg(cmsHero, "linear-gradient(135deg, #0F172A 0%, #1E3A5F 100%)"),
        ...
        <div style={{ maxWidth: 780, margin: "0 auto", position: "relative", zIndex: 1, textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: "clamp(26px, 5vw, 38px)", fontWeight: 900, margin: "0 0 10px", lineHeight: 1.2, color: cmsHero?.textColor ?? undefined }} data-testid="text-page-title">
            {cmsHero?.headline ?? "Lances ao Vivo — Garanta já!"}
          </h1>
          <p style={{ fontSize: "clamp(14px, 2.5vw, 16px)", color: "rgba(255,255,255,0.75)", margin: "0 0 16px", lineHeight: 1.6 }}>
            {cmsHero?.subheadline ?? `${leiloes.filter(l => l.timeLeftSeconds > 0).length} leilões ativos agora — dê lances e conquiste os melhores preços em Caldas Novas!`}
          </p>
      ...
      <PageGallery images={galleryImages} video={galleryVideo} title="Galeria de leilões e destinos" />
    </div>
  )
}
