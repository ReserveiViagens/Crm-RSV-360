    ...
    <div className="rsv-admin-shell" style={{ minHeight: '100vh', background: '#F9FAFB' }}>
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{ flex: 1, padding: '32px 32px 48px' }}>
          ...
          {/* ─── SUPERADMIN — CONTROLE DE ACESSO ────────────────────────── */}
          {currentUser?.role === "superadmin" && (
            <div
              data-testid="card-superadmin-access-control"
              style={{ marginBottom: 24, background: "linear-gradient(135deg, #4C1D95, #7C3AED)", borderRadius: 14, padding: "20px 24px", display: "flex", alignItems: "center", gap: 16, boxShadow: "0 4px 14px rgba(124,58,237,0.35)", cursor: "pointer" }}
              onClick={() => setLocation("/admin/configuracoes-sistema?tab=acesso")}
            >
              ...
              <div style={{ flex: 1 }}>
                <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#fff" }}>Controle de Acesso</p>
                <p style={{ margin: "2px 0 0", fontSize: 13, color: "rgba(255,255,255,0.75)" }}>Aprovar editores e gerenciar perfis verificados</p>
              </div>
              <div style={{ background: "rgba(255,255,255,0.2)", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 600, color: "#fff" }}>
                Acessar →
              </div>
            </div>
          )}
