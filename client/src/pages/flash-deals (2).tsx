...
export default function FlashDealsPage() {
  const { data: cmsContent } = useCMSContent("ofertas")
  const { data: galleryData } = usePageGallery("ofertas")
  ...
  return (
    <div className="rsv-subpage" style={{ background: "#F9FAFB", minHeight: "100vh", paddingBottom: 0 }}>
      ...
      <div data-cms-section="hero" style={{
        background: (cmsHero?.bgColor as string | undefined) ?? "linear-gradient(135deg, #0F1F38 0%, #7C1010 100%)",
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <h1 style={{ fontSize: 30, fontWeight: 900, margin: "0 0 10px", lineHeight: 1.2, color: cmsHero?.textColor ?? undefined }} data-testid="text-page-title">
            {cmsHero?.headline ?? "⚡ Flash Deals — Até 70% OFF"}
          </h1>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.75)", margin: "0 0 24px", lineHeight: 1.6 }}>
            {cmsHero?.subheadline ?? `${FLASH_DEALS.length} deals exclusivos com preços por tempo limitado — garanta antes que acabem!`}
          </p>
          <div style={{
      ...
      <PageGallery images={galleryImages} video={galleryVideo} title="Galeria de flash deals" />
    </div>
  )
}
