...
function QuemSomosHero({ headline, subheadline, bgColor }: { headline?: string; subheadline?: string; bgColor?: string }) {
  return (
    <div style={{
      ...
      <h1 data-testid="text-page-title" style={{ color: "#fff", fontSize: "clamp(24px, 4vw, 40px)", fontWeight: 900, margin: "0 0 12px", lineHeight: 1.2 }}>
        {headline ?? <>A Maior Plataforma de Turismo<br />de Caldas Novas</>}
      </h1>
      <p style={{ color: "rgba(255,255,255,0.82)", fontSize: 16, margin: "0 auto 28px", lineHeight: 1.6, maxWidth: 520 }}>
        {subheadline ?? "Transformando sonhos de viagem em realidade desde 2010"}
      </p>
...
export default function QuemSomosPage() {
  const { data: cmsContent } = useCMSContent("quem-somos")
    ...
    <PublicPageShell
      ...
      heroSlot={
        <div data-cms-section="hero">
          <QuemSomosHero headline={cmsHero?.headline ?? undefined} subheadline={cmsHero?.subheadline ?? undefined} bgColor={getCMSHeroBg(cmsHero, "linear-gradient(135deg, #1e3a5f 0%, #2563EB 100%)")} />
        </div>
      }
      footer={<HomeFooter />}
