...
export default function HoteisPage() {
  const { data: cmsContent } = useCMSContent("hoteis")
  const { data: galleryData } = usePageGallery("hoteis")
  const galleryImages = galleryData?.images ?? []
  const galleryVideo = galleryData?.video ?? null
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage: (cmsHero?.imageUrl as string | undefined) ? `url('${String(cmsHero?.imageUrl)}')` : "url('https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=1600&q=80')",
      ...
      >
        ...
        <div style={{ position: "relative", maxWidth: 780, margin: "0 auto", textAlign: "center" }}>
          ...
          <h1
            ...
            style={{ fontSize: "clamp(26px, 5vw, 40px)", fontWeight: 900, margin: "0 0 12px", lineHeight: 1.2, letterSpacing: -0.5, color: cmsHero?.textColor ?? undefined }}
          >
            {cmsHero?.headline ?? <>Os melhores hotéis de Caldas Novas com <span style={{ color: "#F57C00" }}>até 35% de desconto</span></>}
          </h1>

          <p style={{ fontSize: "clamp(14px, 2.5vw, 16px)", margin: "0 0 24px", opacity: 0.88, lineHeight: 1.6 }}>
            {cmsHero?.subheadline ?? "Hospedagens com piscinas termais, parques aquáticos e café da manhã incluído. Reserva segura e suporte 24h."}
          </p>
      ...
      <PageGallery images={galleryImages} video={galleryVideo} title="Galeria de hotéis e resorts" />
    </CatalogPageShell>
  )
}
