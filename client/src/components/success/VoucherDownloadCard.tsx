...
export function VoucherDownloadCard({
  orderId,
  demo,
  voucherToken,
}: VoucherDownloadCardProps) {
  ...
  async function handleDownload() {
    ...
    try {
      const tokenParam = voucherToken
        ? `?token=${encodeURIComponent(voucherToken)}`
        : "";
      ...
      const msg =
        err instanceof Error ? err.message : "Erro ao baixar voucher";
      setErrorMsg(msg);
      setState("error");
    ...
    <div style={{ marginBottom: 12 }}>
      ...
      {state === "success" && (
        <p
          ...
          data-testid="text-download-success"
        >
          PDF salvo — apresente o voucher no atendimento
        </p>
      )}
    </div>
  );
