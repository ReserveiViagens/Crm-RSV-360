...
export function HeroSection({ headline, subheadline, bgColor, imageUrl }: HeroSectionProps = {}) {
  const heroBackground = imageUrl
    ? `url(${imageUrl}) center/cover no-repeat`
    : bgColor ?? "linear-gradient(135deg, #0F2744 0%, #1E3A5F 50%, #1D4ED8 100%)"
    ...
    >
      ...
      >
        <div>
          ...
          >
            {headline ?? <>Ingressos com desconto para os melhores parques de{" "}<span style={{ color: "#FBBF24" }}>Caldas Novas</span> e{" "}<span style={{ color: "#FBBF24" }}>Rio Quente</span></>}
          </h1>
          ...
          >
            {subheadline ?? "Compare parques, encontre a melhor opção para sua viagem e reserve com suporte rápido no WhatsApp."}
          </p>
