...
## CMS Landing Pages Module (Task #6 — PR 9)

### Overview
Full CMS editor for 15 public landing pages, accessible in `/admin/configuracoes-sistema` under the "Landing Pages" tab.

### Files Added/Modified
- `shared/website-types.ts` — Added `CMSSection`, `CMSPageContent`, `CMSTheme`, `CMSSeo`, `CMS_SECTION_TYPES`, `LANDING_PAGES` const with 15 page definitions
- `client/src/hooks/useCMSContent.ts` — Public hook to fetch CMS content for any page slug from `/api/website/pages/:slug`
- `client/src/components/admin/LandingPagesTab.tsx` — Full CMS editor: section list with drag-up/down, per-section data editors (12 section types), theme editor, SEO editor, live preview iframe, draft/publish save
- `client/src/pages/admin/configuracoes-sistema.tsx` — Added "Landing Pages" tab (3rd tab) using `LandingPagesTab`
- `server/db/seed-website.ts` — Updated to seed all 15 landing pages using `LANDING_PAGES` constant
- `client/src/pages/landing.tsx` — Integrated `useCMSContent("home")` for SEO title and theme color overrides

### 15 Landing Page Slugs
`home`, `hoteis`, `promocoes`, `ingressos`, `atracoes`, `ofertas`, `leiloes`, `excursoes`, `quem-somos`, `caldas-ai`, `grupos`, `perfil`, `mapas`, `contato`, `catalogo`

### 12 CMS Section Types
`hero`, `text`, `image`, `video`, `gallery`, `cta`, `table`, `cards`, `faq`, `svg`, `html`, `divider`

### CMS Content Storage
Each page stores its CMS data in `websitePages.content` (JSONB) as `{ sections: CMSSection[], theme: CMSTheme, seo: CMSSeo }`. The existing public endpoint `GET /api/website/pages/:slug` returns this content.
...
## Admin Mídias — Sincronização completa (Task #7)

### Seed de mídias (`server/db/seed-website.ts`)
- `DEMO_MEDIA` expandido para ~46 entradas cobrindo todas as 15+ páginas públicas
- Páginas cobertas: home, hoteis, atracoes, ingressos, excursoes, catalogo, leiloes, promocoes, caldas-ai, grupos + site identity
- Seed é idempotente (deduplication por `filename`); associa `pageId` via lookup por `pageSlug`
- Placements usados: hero, card, gallery, banner, avatar, icon

### MidiaTab — Filtro por página + Editor Hero Texts (`/admin/configuracoes-sistema`)
- **Dropdown de página** proeminente (sempre visível) filtra a grade de mídias pelo `pageId` da página selecionada
- **`HeroTextEditor`** colapsível: seleciona uma página → edita headline + subheadline → salva via `updatePage` (atualiza `content.hero.{headline,subheadline}`)
- Badge exibe o nome da página selecionada na contagem de mídias

### Swap → CMS Hero imageUrl (`server/routes/admin-website.routes.ts`)
- Ao fazer swap de uma mídia com `placement === "hero"` e `pageId` vinculado, o endpoint chama `updatePageHeroImageUrl(pageId, newUrl)` automaticamente
- `updatePageHeroImageUrl` está em `server/services/admin-website.service.ts` — atualiza `content.hero.imageUrl` da página preservando os demais campos do hero

### Fluxo completo
1. Admin acessa aba Mídias → filtra por página
2. Encontra a hero image da página → faz swap com nova imagem
3. Backend: salva novo arquivo, atualiza registro `website_media`, atualiza `content.hero.imageUrl` na página
4. Frontend: na visita pública, `useCMSContent(slug)` retorna o hero atualizado → `getCMSHeroBg()` usa o `imageUrl` como background da section hero