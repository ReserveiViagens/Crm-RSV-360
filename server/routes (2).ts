...
import publicWebsiteRoutes from "./routes/public-website.routes.js";
import { requireAdmin as requireAdminWebsite, requireEditor } from "./middleware/admin-auth.middleware.js";
...
export async function registerRoutes(
  ...
  // ─── ADMIN WEBSITE CMS ───────────────────────────────────────────────────
  app.use("/api/admin/website", requireEditor, adminWebsiteRoutes);
  ...
  app.post("/api/auth/register", async (req: Request, res: Response) => {
    ...
    const user = await storage.createUser({
      ...
      role: "user", ativo: true, googleId: "", fotoUrl: "", provider: "local",
    });
  ...
  // ─── CONTROLE DE ACESSO — ACCESS REQUESTS ───────────────────────────────────

  app.get("/api/access-requests/me", async (req: Request, res: Response) => {
    if (!req.session.userId) return res.status(401).json({ message: "Não autenticado" });
    const existing = await storage.getAccessRequestByUserId(req.session.userId);
  ...
  app.post("/api/access-requests", async (req: Request, res: Response) => {
    ...
    if (user.role === "admin" || user.role === "superadmin" || user.role === "editor") {
      return res.status(400).json({ message: "Você já possui acesso de editor ou superior." });
    }
    const existing = await storage.getPendingAccessRequestByUserId(req.session.userId);
    if (existing) {
      return res.status(409).json({ message: "Você já possui uma solicitação pendente." });
    }
    const { message } = req.body as { message?: string };
    const req2 = await storage.createAccessRequest({
  ...
  app.get("/api/admin/access-requests", requireAdminWebsite, async (req: Request, res: Response) => {
    const { status } = req.query as { status?: string };
    const validStatuses = ["pending", "approved", "rejected"];
    const filterStatus = status && validStatuses.includes(status) ? (status as "pending" | "approved" | "rejected") : undefined;
    const requests = await storage.listAccessRequests(filterStatus);
    const enriched = await Promise.all(
      requests.map(async (r) => {
        const user = await storage.getUser(r.userId);
        return { ...r, user: user ? { id: user.id, nome: user.nome, email: user.email, role: user.role } : null };
      })
    );
  ...
  app.patch("/api/admin/access-requests/:id", requireAdminWebsite, async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    const allRequests = await storage.listAccessRequests();
    ...
    if (accessReq.status !== "pending") {
      return res.status(409).json({ message: "Esta solicitação já foi processada." });
    }
    const newStatus = action === "approve" ? "approved" : "rejected";
    const updated = await storage.updateAccessRequest(id, {
      status: newStatus,
      reviewedBy: adminUser?.nome ?? "Admin",
      reviewedAt: new Date().toISOString(),
    });
    if (action === "approve") {
      await storage.updateUser(accessReq.userId, { role: "editor" });
    }
    return res.json(updated);
  });

  app.delete("/api/admin/access-requests/:id", requireAdminWebsite, async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  app.get("/api/admin/users", requireAdminWebsite, async (req: Request, res: Response) => {
    ...
    return res.json({ items: safeList });
  });

  app.patch("/api/admin/users/:id/role", requireAdminWebsite, async (req: Request, res: Response) => {
    const { id } = req.params;
    const { role } = req.body as { role?: string };
    const validRoles = ["admin", "editor", "LIDER", "cliente", "user"];
    if (!role || !validRoles.includes(role)) {
      return res.status(400).json({ message: "Role inválida." });
    }
    const updated = await storage.updateUser(id, { role });
    if (!updated) return res.status(404).json({ message: "Usuário não encontrado." });
    return res.json({ id: updated.id, nome: updated.nome, email: updated.email, role: updated.role, ativo: updated.ativo ?? true });
  });

  app.patch("/api/admin/users/:id/ativo", requireAdminWebsite, async (req: Request, res: Response) => {
    const { id } = req.params;
    const { ativo } = req.body as { ativo?: boolean };
    if (typeof ativo !== "boolean") {
      return res.status(400).json({ message: "Campo 'ativo' deve ser boolean." });
    }
    const updated = await storage.updateUser(id, { ativo });
    if (!updated) return res.status(404).json({ message: "Usuário não encontrado." });
    return res.json({ id: updated.id, nome: updated.nome, email: updated.email, role: updated.role, ativo: updated.ativo });
  });
          ...
          if (!user) {
            ...
            if (user) {
              ...
              user = await storage.createUser({
                ...
                email, telefone: "", cpf: "", role: "user", ativo: true,
