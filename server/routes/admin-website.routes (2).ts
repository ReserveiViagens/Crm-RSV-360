...
const uploadRateLimit = rateLimit({
  ...
  legacyHeaders: false,
  message: { success: false, error: "Limite de upload atingido. Tente novamente em 1 minuto.", code: "RATE_LIMIT_EXCEEDED" },
});
...
const deleteRateLimit = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  message: { success: false, error: "Limite de exclusão atingido. Tente novamente em 1 minuto.", code: "RATE_LIMIT_EXCEEDED" },
});
...
function getActor(req: Request): { actorId: string; actorName: string } {
  if (req.adminUser) {
    return {
      ...
      actorName: req.adminUser.nome ?? req.adminUser.username ?? req.adminUser.id,
    };
  }
  const userId = (req.session as { userId?: string })?.userId ?? "system";
  return { actorId: userId, actorName: userId };
}
...
router.patch("/pages/:id", handleUpdatePage);
router.put("/pages/:id", handleUpdatePage);
...
router.post("/media/external-video", uploadRateLimit, async (req: Request, res: Response) => {
  const { url, pageId, placement, altText } = req.body ?? {};

  if (!url || typeof url !== "string") {
    return res.status(400).json({ success: false, error: "URL obrigatória", code: "VALIDATION_ERROR" });
  }
  ...
  if (!isYouTubeOrVimeo) {
    return res.status(422).json({
      success: false,
      error: "Apenas URLs do YouTube ou Vimeo são aceitas",
      code: "INVALID_URL",
    });
  }
  ...
  try {
    const media = await createExternalMediaRecord({
      url,
      type: "video",
...
router.get("/media/:id", async (req: Request, res: Response) => {
  ...
  const media = await getMediaById(parsed.data.id);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (media.placement === "hero" && media.pageId) {
        await updatePageHeroImageUrl(media.pageId, media.url);
      }
...
router.delete("/media/:id", deleteRateLimit, async (req: Request, res: Response) => {
  const paramParsed = mediaIdParamValidator.safeParse(req.params);
  if (!paramParsed.success) {
    return res.status(400).json({ success: false, error: "ID inválido", code: "VALIDATION_ERROR" });
  }
...
router.get("/audit", async (req: Request, res: Response) => {
  const parsed = auditQueryValidator.safeParse(req.query);
  if (!parsed.success) {
    return res.status(400).json({
      success: false,
      error: parsed.error.errors[0]?.message ?? "Parâmetros inválidos",
  ...
  const { entity, entityId, action, limit, page } = parsed.data;
  const result = await getAuditLogs({ entity, entityId, action, limit, page });
  return res.json({ success: true, ...result });
});
