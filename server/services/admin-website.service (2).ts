...
export async function updatePage(
  ...
  await writeAudit("page", id, "update", actorId, actorName, { ...input as Record<string, unknown>, slug: row.slug });
  return toPageDto(row);
}
...
export async function publishPage(
  ...
  await writeAudit("page", id, "publish", actorId, actorName, { slug: row.slug });
  return toPageDto(row);
}
...
export async function unpublishPage(
  ...
  await writeAudit("page", id, "unpublish", actorId, actorName, { slug: row.slug });
  return toPageDto(row);
}

/* ─── CMS Hero ───────────────────────────────────────────────────────────── */

export async function updatePageHeroImageUrl(
  pageId: string,
  imageUrl: string
): Promise<boolean> {
  const [existing] = await db
    .select({ id: websitePages.id, content: websitePages.content })
    .from(websitePages)
  ...
  const updatedContent = { ...content, hero: { ...hero, imageUrl } };
