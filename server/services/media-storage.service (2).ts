...
/* ─── Ensure uploads directory exists (local mode) ─────────────────────────── */
...
async function s3Upload(localPath: string, key: string, mimetype: string): Promise<void> {
...
async function s3Delete(key: string): Promise<void> {
  ...
  try {
    ...
    console.warn(`[media-storage] S3 delete failed for key ${key}:`, err);
  }
}
...
export function buildMediaUrl(filename: string): string {
  const cdnBase = process.env.CDN_BASE_URL ?? process.env.MEDIA_CDN_BASE_URL;
  if (cdnBase) {
...
/* ─── Delete file from disk or S3 (best-effort, non-fatal) ─────────────────── */
...
export function deleteFileFromDisk(filename: string): void {
  ...
  try {
    ...
    console.warn(`[media-storage] Could not delete local file: ${filePath}`);
  }
}
...
export function deleteStoredFile(filename: string): void {
  if (isS3Mode()) {
    ...
    s3Delete(key).catch((err) =>
      console.warn(`[media-storage] S3 delete enqueued error for ${key}:`, err)
    );
  } else {
...
export async function persistMediaRecord(
  ...
  if (isS3Mode()) {
    const key = `website/${file.filename}`;
    try {
      await s3Upload(file.path, key, file.mimetype);
    } catch (err) {
      deleteFileFromDisk(file.filename);
      throw Object.assign(new Error("Falha ao enviar arquivo para o storage"), {
        cause: err,
  ...
  try {
    ...
    if (!isS3Mode()) deleteFileFromDisk(file.filename);
    throw err;
  }
...
/* ─── Create external media record (URL-only, no file upload) ─────────────── */

export async function createExternalMediaRecord(params: {
  url: string;
  type: "image" | "video";
  placement: string;
  pageId?: string | null;
  ...
  const filename = `external-${Date.now()}-${urlObj.hostname.replace(/\./g, "-")}`;

  const rows = await db
    .insert(websiteMedia)
    .values({
      type: params.type,
      placement: params.placement as WebsiteMediaRow["placement"],
      status: "active",
      filename,
      originalName: params.url,
      mimetype: params.type === "video" ? "video/external" : "image/external",
      sizeBytes: 0,
      url: params.url,
  ...
  await writeMediaAudit({
    ...
    diff: { url: params.url, type: params.type, placement: params.placement },
  });
...
export async function swapMediaFile(
  ...
  if (isS3Mode()) {
    const key = `website/${newFile.filename}`;
    try {
      await s3Upload(newFile.path, key, newFile.mimetype);
    } catch (err) {
      deleteFileFromDisk(newFile.filename);
      throw Object.assign(new Error("Falha ao enviar arquivo para o storage"), {
        cause: err,
  ...
  try {
    ...
    if (!isS3Mode()) deleteFileFromDisk(newFile.filename);
    throw err;
  }

  deleteStoredFile(oldFilename);
