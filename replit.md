...
- **Excursão em Grupo** (`/viagens-grupo`): 5-tab primary navigation (Início, Roteiro, Planejar, Votar, Chat). Each tab isolates related content: Início shows costs & timeline; Roteiro shows official itinerary governance; Planejar has wizard + AI planner + hotel selector; Votar shows hotel voting; Chat provides inline group chat with AI assistant. Sticky tab bar with underline indicator. FloatingChat FAB removed (replaced by Chat tab). Tab bar uses `data-testid="primary-tab-{0-4}"`
